...
def build_nfa(postfix: list[Token]) -> NFA:
    ...
    for token in postfix:
        if token.type is TokenType.SYMBOL:
            ...
            pila.append(_char_class(constructor, fabrica, token.symbols))
...
def _char_class(
    constructor: NFABuilder, fabrica: StateFactory, symbols: tuple[str, ...]
) -> _Fragment:
    """``[abc]``: ``s --a,b,c--> e``.

    Equivale a la union ``(a|b|c)`` pero sin los estados y transiciones epsilon
    intermedios: un solo par de estados con una transicion por simbolo. Asi una
    clase como ``[a-zA-Z0-9]`` no agrega 124 estados al AFN.
    """
    if not symbols:
        raise AutomatonError("la clase de caracteres no contiene ningun simbolo")
    inicio = fabrica.new_state()
    fin = fabrica.new_state()
    for simbolo in symbols:
        constructor.add_transition(inicio, simbolo, fin)
    return _Fragment(inicio, fin)
...
def _concatenation(
    ...
    """``rs``: el final del primero enlaza con el inicio del segundo."""
