...
from ..constants import COMMENT_PREFIX, EPSILON_INPUTS
...
class WordLine:
    ...
    ``ε`` (o ``^``) en el archivo representa la cadena vacia, de modo que puede escribirse
...
def read_words(path: Path) -> list[WordLine]:
    ...
    Una linea con ``ε`` (o ``^``) representa la cadena vacia.
    ...
    return [
        WordLine(line_number=numero, word="" if texto in EPSILON_INPUTS else texto)
